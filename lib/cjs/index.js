...
const getOne = async ({ conn, tableName, condition = {}, }) => {
    const result = await (0, exports.get)({
        conn,
        tableName,
...
const getSearch = async ({ conn, tableName, condition = {}, }) => {
    ...
    // get result.data
    const data = await (0, exports.get)({
        conn,
        tableName,
        condition: {
            ...param,
    ...
    // check is_more
    const isMoreResp = await (0, exports.get)({
        conn,
        tableName,
        condition: {
            ...param,
    ...
    let total;
    if (_getTotal !== false) {
        total = await (0, exports.count)({ conn, tableName, condition: { param } });
    }
    return {
        data,
        is_more,
...
const generateCode = async ({ conn, tableName, condition = {}, key = "code", length = 4, }) => {
    ...
    const prevResult = await (0, exports.getOne)({ conn, tableName, condition: query });
    // define new code
    ...
    do {
        ...
        check = await (0, exports.getOne)({ conn, tableName, condition: checkQuery });
 ...
 * @returns {result: {rows, fields}}
 */
const edit = async ({ conn, tableName, condition, values, }) => {
    // check conditions is not empty
    if (!(condition && lodash_1.default.isPlainObject(condition) && !lodash_1.default.isEmpty(condition))) {
    ...
    // get origin data
    const originList = (await (0, exports.get)({
        conn,
        tableName,
        condition,
    }));
    // prepare sql for update
    const conditionString = lodash_1.default.chain(condition)
        .keys()
        .map((key) => `\`${key}\` = ?`)
        .value()
        .join(" AND ");
    const setString = lodash_1.default.chain(filterValues)
        ...
        .value()
        .join(", ");
    const finalValues = [...lodash_1.default.values(filterValues), ...lodash_1.default.values(condition)];
    ...
    try {
        ...
        const ps = lodash_1.default.map(originList, (data) => (0, exports.getOne)({
            conn,
            tableName,
            condition: {
                id: data.id,
