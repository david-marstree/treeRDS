...
const primaryKey = ({ name }) => (0, exports.field)({
    name,
    type: types_1.MySQLFieldType.INT,
    primaryKey: true,
    defaultValue: 0,
    index: [name],
});
 ...
 * @returns fieldObject
 */
const intField = ({ name, defaultValue = 0, index, }) => (0, exports.field)({ name, type: types_1.MySQLFieldType.INT, defaultValue, index });
exports.intField = intField;
/**
 * @abstract define varchar field for table in database
 * @returns fieldObject
 */
const varcharField = ({ name, defaultValue = null, index, }) => (0, exports.field)({ name, type: types_1.MySQLFieldType.VARCHAR, defaultValue, index });
 ...
 * @returns fieldObject
 */
const longtextField = ({ name }) => (0, exports.field)({ name, type: types_1.MySQLFieldType.LONGTEXT });
exports.longtextField = longtextField;
/**
 * @abstract define double field for table in database
 * @returns fieldObject
 */
const doubleField = ({ name, defaultValue = 0, index, }) => (0, exports.field)({ name, type: types_1.MySQLFieldType.DOUBLE, defaultValue, index });
...
const _getType = (field) => {
    if (field.type === types_1.MySQLFieldType.INT) {
        return "int(11)";
    }
    else if (field.type === types_1.MySQLFieldType.VARCHAR) {
        return "varchar(255)";
    }
    else if (field.type == types_1.MySQLFieldType.LONGTEXT) {
        return "longtext";
    }
    else if (field.type === types_1.MySQLFieldType.DOUBLE) {
        return "double";
    }
    return false;
};
